...
if [ ! -z $step04 ]; then
    echo "## Stage 04: Make Json Labels"
    # make json labels
    data2json.sh --nj $nj --cmd "${cmd}" --feat $data/${train_set}/dump/feats.scp --trans_type char \
        $data/${train_set} ${dict} > ${data}/${train_set}/${train_set}_${bpemode}_${vocab_size}.json

    for x in $train_set $valid_set $recog_set;do 
        data2json.sh --nj $nj --cmd "${cmd}" --feat $data/${x}/dump_${train_set}/feats.scp --trans_type char \
...
if [ ! -z $step6 ]; then
    ...
    train_set=train
    nj=50
    for expname in ${train_set}_6enc_age_estimation_withSpecAug_${lossType}_${backend}; do
    expdir=$exp/${expname}
    
    for max_epoch in 5 10 15 20 30 40 50;do
    for recog_set in $valid_set $recog_set;do
    # 
    for use_valbest_average in false true; do
    ...
    echo "## step 06: Decoding Finished"
fi
