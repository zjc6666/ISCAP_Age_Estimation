...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False):
        ...
        # logging.warning("this is wav2vec")
        ...
        return result
    def exrtactEncoderFeature(self, source, padding_mask=None, mask=True, features_only=False): 
        # logging.warning("this is wav2vec")
        # logging.warning("wav2vec2 model inut size()===>" + str(source.size()))
        if self.feature_grad_mult > 0:
            features = self.feature_extractor(source)
            if self.feature_grad_mult != 1.0:
                features = GradMultiply.apply(features, self.feature_grad_mult) 
        ...
        x = self.encoder.extractAllLayerfeatures(x, padding_mask=padding_mask)
        return x
...
class TransformerEncoder(nn.Module):
    ...
    def extract_features(self, x, padding_mask=None):
        ...
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        # print("Encoder layers lenght===>", len(self.layers))
        layer_results = []
    ...
    def extractAllLayerfeatures(self, x, padding_mask=None):
