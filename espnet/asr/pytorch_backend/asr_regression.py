...
def recog(args):
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    if(args.add_xvector == 0):
                        prediction_label = model.recognize(feat)
                    else:
                        prediction_label = model.recognize(feat, name)
                logging.warning('Utt ' + ', Label = ' + str(prediction_label))
                new_js[name] = float(prediction_label)
    ...
    with open(args.result_label, 'w') as f:
        for utt in new_js.keys():
            f.write(utt + "\t" + str(new_js[utt]) + "\n")
